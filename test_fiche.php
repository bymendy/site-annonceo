...
$pdoStatement = $pdo->prepare('SELECT * FROM annonce WHERE id_annonce = :id_annonce');
...
$pdoStatement2 = $pdo->prepare('SELECT * FROM membre INNER JOIN annonce ON membre_id=id_membre  WHERE id_annonce = :id_annonce');
$pdoStatement2->bindValue(":id_annonce", $_GET['id_annonce'], PDO::PARAM_INT);
$pdoStatement2->execute();
$arrayMembre1 = $pdoStatement2->fetch(PDO::FETCH_ASSOC);

$pdoStatement3 = $pdo->prepare('SELECT * FROM photo INNER JOIN annonce ON photo_id=id_photo  WHERE id_annonce = :id_annonce');
...
$pdoStatement4 = $pdo->prepare('SELECT * FROM annonce WHERE categorie_id = :categorie_id AND id_annonce != :id_annonce');
...
$pdoStatement1 = $pdo->prepare('SELECT * FROM note WHERE membre_id2 = :id_membre');
...
if (isset($_POST['envoyercommentaire']) && $_POST['envoyercommentaire']) {
    $pdoStatement5 = $pdo->prepare('INSERT INTO commentaire (membre_id, annonce_id, commentaire, date_enregistrement) VALUES (:membre_id, :annonce_id, :commentaire, :date_enregistrement)');
...
if (isset($_POST['envoyernote']) && $_POST['envoyernote']) {
    $pdoStatement6 = $pdo->prepare('INSERT INTO note (membre_id1, membre_id2, note, avis, date_enregistrement) VALUES (:membre_id1, :membre_id2, :note, :avis, :date_enregistrement)');
...
$pdoStatement7 = $pdo->prepare('SELECT * FROM commentaire WHERE annonce_id = :annonce_id');
